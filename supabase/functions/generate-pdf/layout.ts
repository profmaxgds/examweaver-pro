...
export function generateExamHTML(exam: ExamData, questions: Question[], version: number, includeAnswers: boolean, studentInfo?: StudentInfo): string {
    ...
    // Estilos CSS que incorporam as âncoras para deteção por projeção
    const styles = `
        @import url('https://fonts.googleapis.com/css2?family=Times+New+Roman&display=swap');
        body { font-family: 'Times New Roman', Times, serif; font-size: 11pt; line-height: 1.4; margin: 0; padding: 1.5cm; color: #000; background-color: #fff; }
        .page-container { width: 100%; max-width: 18cm; margin: auto; }
        ...
        /* --- CSS do Gabarito (AJUSTADO PARA ÂNCORAS) --- */
        .answer-sheet-container {
            margin-bottom: 25px;
            display: flex;
            gap: 0.3cm;
        ...
        .column-divider { width: 1.5px; background-color: #000; margin: 0 10px; }
        .answer-options-header { display: flex; margin-bottom: 4px; padding-left: 34px; height: 15px; align-items: center; }
        .answer-options-header span { width: 16px; text-align: center; font-size: 9pt; font-weight: bold; margin: 0 2.5px; }
        .answer-row { display: flex; align-items: center; margin-bottom: 4px; height: 15px; /* Altura fixa para alinhamento */ }
        .answer-row .q-number { font-weight: bold; margin-right: 6px; font-size: 10pt; width: 30px; text-align: right; }
        .answer-row .options-bubbles { display: flex; align-items: center; }
        .answer-row .bubble { width: 11px; height: 11px; border: 1px solid #000; margin: 0 2.5px; }
        .answer-row .essay-indicator { font-size: 9pt; color: #555; font-style: italic; margin-left: 5px; }

        /* --- CSS DO MARCADOR DE ÂNCORA (NOVO E MELHORADO) --- */
        .anchor-container { position: absolute; top: 0; left: 0; right: 0; bottom: 0; pointer-events: none; }
        .anchor-marker { position: absolute; background-color: #000 !important; -webkit-print-color-adjust: exact !important; }
        
        .vertical-anchors { position: absolute; top: 37px; left: 8px; display: flex; flex-direction: column; gap: 4px; }
        .vertical-anchors .anchor-marker { width: 8px; height: 15px; }
        
        .horizontal-anchors { position: absolute; bottom: 8px; left: 40px; display: flex; flex-direction: row; gap: 6px; }
        .horizontal-anchors .anchor-marker { width: 16px; height: 8px; }
        ...
        @media print { 
            body { -webkit-print-color-adjust: exact; }
            .anchor-marker, .correct-answer-highlight { 
                -webkit-print-color-adjust: exact !important; 
                color-adjust: exact !important; 
            }
        }
    ...
    const generateAnchors = () => {
        ...
        // Âncoras Verticais: 1 para o cabeçalho + 1 para cada questão na coluna mais longa
        const numVerticalAnchors = questionsPerColumn + 1;

        // Âncoras Horizontais: 5 alternativas para cada coluna
        const numHorizontalAnchors = 5 * numCols;

        let verticalAnchorsHTML = `<div class="vertical-anchors">`;
        for(let i = 0; i < numVerticalAnchors; i++) {
            verticalAnchorsHTML += `<div class="anchor-marker"></div>`;
        }
        verticalAnchorsHTML += `</div>`;
        
        let horizontalAnchorsHTML = `<div class="horizontal-anchors">`;
        for(let i = 0; i < numHorizontalAnchors; i++) {
            horizontalAnchorsHTML += `<div class="anchor-marker"></div>`;
        }
        horizontalAnchorsHTML += `</div>`;
        
        return verticalAnchorsHTML + horizontalAnchorsHTML;
    };
